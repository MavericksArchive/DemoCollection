...
from transformers import AutoModel, AutoTokenizer, AutoConfig, T5EncoderModel, T5Config, MistralConfig, LongT5EncoderModel, LongT5Config

# sys.path.append('..')

from MyPooling import MyPooling
from my_utils import pairwise_dot_score 
import numpy as np 
...
class MyBIModel(nn.Module):
    def __init__(self, model_name, bnb_config, train_type="sim", device="cuda"):
        ...
        if isinstance(self.config ,  LongT5Config):
            ...
            self.auto_model = LongT5EncoderModel.from_pretrained(model_name, config=self.config).to(self.device)
            print("Initialize model LongT5EncoderModel")
            self.max_length=2048
            print("self.max_length", self.max_length)
        elif isinstance(self.config , T5Config):
            T5EncoderModel._keys_to_ignore_on_load_unexpected = ["decoder.*"]
            self.auto_model = T5EncoderModel.from_pretrained(model_name, config=self.config).to(self.device)
        ...
        self.pooling_model = MyPooling(self.auto_model.config.hidden_size).to(device)
    ...
    def forward(self, tokenized_inputs, label):
        if self.train_type == "sim":
            tokenized_inputs[0]['token_embeddings']=self.auto_model(**tokenized_inputs[0], return_dict=False)[0]
            tokenized_inputs[1]['token_embeddings']=self.auto_model(**tokenized_inputs[1], return_dict=False)[0]
            ...
            tokenized_inputs[0]['token_embeddings']=self.auto_model(**tokenized_inputs[0], return_dict=False)[0]
            tokenized_inputs[1]['token_embeddings']=self.auto_model(**tokenized_inputs[1], return_dict=False)[0]
            tokenized_inputs[2]['token_embeddings']=self.auto_model(**tokenized_inputs[2], return_dict=False)[0]
    ...
    def encode(self, sentences, batch_size=8, normalize_embeddings= True, show_bar=True):
        ...
        for start_index in tqdm(range(0, len(sentences), batch_size), disable = not show_bar):
                with torch.no_grad():
                    ...
                    tokenized = self.tokenizer(sentences_batch, padding=True, truncation=True, return_tensors='pt').to(self.device)
    ...
    def smart_batching_collate(self, batch):
        ...
        for idx in range(num_texts):
            tokenized = self.tokenizer(texts[idx], return_tensors="pt",padding=True, max_length=self.max_length, truncation=True).to(self.device)
