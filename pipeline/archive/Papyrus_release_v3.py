...
curl -X POST 'http://ip-10-0-0-93.us-west-2.compute.internal:2222/papyrusGen?conversation_id=25141c36-9622-4726-a4fa-04d7d6403a45\&message_id=81764c5d-920c-4cbf-a3e8-f04ea26e5790&source=netlens\&dryrun=True\&debug=True' -H 'Content-Type: application/json' -d '{"user_text": "I have 187 devices that show as potentially vulnerable to the PSIRT cisco-sa-iosxe-webui-privesc-j22SaA4z. How can I confirm vulnerability to this PSIRT that allows a remote, unauthenticated attacker to create an account on an affected device with privilege level 15 access and can then use that account to gain control of the affected device?", "nodes_run_data": []}'
...
def testPipeline():
    ...
    agg_intention = {
        'goal': f"The goal of the user is \'{topic_output_json.get('goal')}\'\n" if topic_output_json.get('goal') else '',
        'main_question': f"The main question the user has is \'{topic_output_json.get('main_question')}\'" if topic_output_json.get('main_question') else '',
        'major_problem': f"The major probleme the user is facing is \'{topic_output_json.get('major_problem')}\'" if topic_output_json.get('major_problem') else '',
        'extracted_entity': f"The extracted entity is \'{topic_output_json.get('extracted_entity')}\'" if topic_output_json.get('extracted_entity') else '',
        'system_message': f"The system_message is \'{topic_output_json.get('system_message')}\'" if topic_output_json.get('system_message') else '',
    }
    ...
    try:
        if str(debug) == 'True':
            pipeline_output = {
                ...
                "[DEBUG] intention": topic_output_json.get('intention', '')
            }
        else:
            pipeline_output = {
                "conversation_id": conversation_id,     # pass through
