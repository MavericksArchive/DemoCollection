...
def ask(topic_module_output, context):
    ...
    prompt = PromptTemplate(template=template, input_variables=["context", "question"])
    chain = prompt | tmp.llm | StrOutputParser() 
    response = chain.invoke({"context": topic_module_output['situation'], "question": question})
    return response
...
def main():
    ...
    psirt_title, psirt_link = google_search(psirt_term)
    
    # get the search result
    context = load_parse_web_doc(psirt_link)

    # ask GPT-35
    response = ask(topic_module_output, context)
        
    topic_module_output['solutions'] = response
